...
#define S_FUNCTION_NAME  THREE_COMBINER

#include "simstruc.h"

#include "THREE_COMBINER.h"      //Definiciones del proceso y del modelo.
...
{	                              
    ...
    ssSetInputPortWidth(S, IP_INFLUX3, ANCHURAPUERTO3ENTRADA); ssSetInputPortDirectFeedThrough(S, 2, 1);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	InputRealPtrsType uPtrs2 = ssGetInputPortRealSignalPtrs(S,2); /* Consignas */
	...
	y[0] = Q_IN1 + Q_IN2 + Q_IN3;

	for (i=1;i<ANCHURAPUERTO1SALIDA ;i++) { y[i] = (Q_IN1*(*uPtrs0[i]) + Q_IN2*(*uPtrs1[i]) +  Q_IN3*(*uPtrs2[i]))/(Q_IN1 + Q_IN2 + Q_IN3) ;}
